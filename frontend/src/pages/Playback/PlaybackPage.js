...
const PlaybackPage = () => {
    ...
    const player_id=useSelector(state=>state.game.current_player_id)
    useEffect(() => {
    ...
    useEffect(() => {
        if (currentFrame < playbackData.length - 1) {
            const timeout = setTimeout(() => {
               ...
               let findPlayer = players.find(player => player.id === player_id);
                setOffset({x:findPlayer.segments[0].x-findPlayer.segments[1].x,y:findPlayer.segments[0].y-findPlayer.segments[1].y})
    ...
    return (
        <div className="playback-container">
            <Map offset={offset} />
            <Snake players={players} GameCanvas={canvasRef}/>;
        </div>
    );
};
